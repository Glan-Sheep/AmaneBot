...
createCommand({
  ...
  async execute(): Promise<EmbedBuilder> {
    ...
    const data = await axiod.get(`https://kenkyu392.github.io/holidays-jp/v1/${this_year}/date.json`);
    let count = 0;
    for (let i = 0; i < data.data.holidays.length; i++) {
      ...
      if (element_date > this_date) {
        embeds.addFields({ name: `${element.i18n["ja-JP"]}`,
        value:`**${element_date.getMonth()+1}月${element_date.getDate()}日${[ "日", "月", "火", "水", "木", "金", "土" ][element_date.getDay()]}曜日**`
        , inline: true })
        count++;
        if(count === 3) break;
      }
