...
createCommand({
  ...
  async execute(_interaction, args): Promise<EmbedBuilder> {
    ...
    if (country !== "JP") {
      ...
      const wea_data = await axiod.get(
        `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=temperature_2m_max,temperature_2m_min,precipitation_sum&timezone=Asia%2FTokyo`
      );
      for (let i = 0; i < wea_data.data.daily.time.length; i++) {
        embeds.addFields({
          name: `[${wea_data.data.daily.time[i]}]`,
